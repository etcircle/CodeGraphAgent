...
class GraphBuilder:
    ...
    def update_file_in_graph(self, path: Path, repo_path: Path, imports_map: dict):
        ...
        if path.exists():
            ...
            if "error" not in file_data:
                ...
                warning_logger(
                    f"Falling back to minimal file node for {file_path_str}: {file_data['error']}"
                )
    ...
    def _resolve_index_root(self, path: Path) -> Path:
        """Return the directory root used for relative-path ignore evaluation."""
        resolved = path.resolve()
        return resolved if resolved.is_dir() else resolved.parent

    def _load_ignore_spec(self, path: Path, create_default_cgcignore: bool = False) -> Tuple[pathspec.PathSpec, Path]:
        """Build the ignore matcher used for repository indexing/verification."""
        cgcignore_path = None
        ignore_root = self._resolve_index_root(path)
        ...
        elif create_default_cgcignore:
            new_cgcignore = ignore_root / ".cgcignore"
            try:
        ...
        return spec, ignore_root

    def _load_ignored_dir_names(self) -> set[str]:
        """Return configured directory names to skip during indexing."""
        ignore_dirs_str = get_config_value("IGNORE_DIRS") or ""
        return {d.strip().lower() for d in ignore_dirs_str.split(',') if d.strip()}

    def collect_indexable_file_paths(self, path: Path, create_default_cgcignore: bool = False) -> list[Path]:
        """Enumerate the filesystem files this repo path should contribute to the graph."""
        path = path.resolve()
        spec, ignore_root = self._load_ignore_spec(path, create_default_cgcignore=create_default_cgcignore)
        files = [f for f in (path.rglob("*") if path.is_dir() else [path]) if f.is_file()]

        ignore_dirs = self._load_ignored_dir_names()
    ...
    def get_indexed_file_paths(self, repo_path: Path) -> set[str]:
        """Return all file paths currently attached to the repository node."""
        repo_path_str = str(repo_path.resolve())
        with self.driver.session() as session:
            result = session.run(
                """
            ...
            return {record["path"] for record in result if record.get("path")}

    def verify_repository_index(self, path: Path, sample_limit: int = 20) -> Dict[str, Any]:
        """Compare expected file inventory vs indexed File nodes for a repository path."""
        path = path.resolve()
        expected_paths = {str(file_path.resolve()) for file_path in self.collect_indexable_file_paths(path)}
        indexed_paths = self.get_indexed_file_paths(path)
        ...
        return {
            ...
            "missing_paths": missing_paths[:sample_limit],
            "extra_paths": extra_paths[:sample_limit],
            "is_clean": not missing_paths and not extra_paths,
        }
    ...
    async def build_graph_from_path_async(
        ...
        try:
            ...
            files = self.collect_indexable_file_paths(path, create_default_cgcignore=True)
            if job_id:
